...
//--------------------------------- TRAITEMENTS PHP ---------------------------------//
if(isset($_GET['id'])){
    $resultat = executeRequete("SELECT * FROM ingredients_data WHERE id = '$_GET[id]'");
}
if($resultat->num_rows <= 0){
    header("location:all_ingredients_page.php");
    exit();
}
...
$contenu .= '<div class="grande_div" style="display: flex; width: 732px; margin: 0 auto; margin-top:20px; margin-bottom:20px;">';
    $contenu .= '<div class="info_img">';
        $contenu .= "<img style='margin-bottom:20px; background-color: #eae9e4;border: 5px solid #20af94; border-radius: 15px; padding: 5px;' src='$recipe[image_links]' ='268' height='268'>";
    $contenu .= "</div>";
    $contenu .= '<div class="info_div" style="margin-left:30px;color: #eae9e4;">';
        ...
        $contenu .= '<p class="p_ingredient_titre" style="background-color: #20af94; font-size: 40px; margin:0px;">Type</p>';

        $contenu .= '<p class="p_ingredient" style="font-size: 16px;">' . $recipe['type'] . '</p>';
